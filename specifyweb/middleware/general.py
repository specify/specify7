...
from django.views.decorators.http import require_http_methods as raw_require_http_methods

REQUEST_METHOD = Literal['HEAD', 'GET', 'POST', 'PUT', 'DELETE', 'OPTIONS']
...
def serialize_django_obj(django_obj: frozenset[models.QuerySet | models.Model]) -> list[dict[str, Any]] or dict[str, Any]:
    ...
    if isinstance(django_obj, set):
        for object in django_obj:
            if isinstance(object, models.QuerySet):
                ...
                raw_queryset = json.loads(
                    serializers.serialize('json', object))
                parsed_data.append(
                    {
