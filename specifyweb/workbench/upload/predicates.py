...
class DjangoPredicates(NamedTuple):
    ...
    def reduce_for_to_one(self):
        if (
            not self.filters
            and not self.to_remove
            and not isinstance(self, SkippablePredicate)
        ):
    ...
    def reduce_for_to_many(self, uploadable: Any):  # type: ignore
    ...
    def get_cache_key(self, basetable_name: str = None) -> str:
        filters = [
            ...
            (
                key,
                (
                    tuple([value.get_cache_key() for value in values])
    ...
    def _smart_apply(
        ...
        _get_field_name = lambda raw_name: (
            raw_name if path is None else (path + "__" + raw_name)
        )

        # it's useless to match on nothing. caller should be aware of that
        assert (
            self.filters is not None and self.filters
        ), "trying to force match on nothing"
        ...
        def _reduce_by_key(rel_name: str):
            ...
            def _reduce(
                ...
                if current.to_remove:
                    assert (
                        not current.filters
                    ), "Hmm, we are trying to filter on something we'll remove. How??"
                    ...
                    to_remove_node = add_to_remove_node(
                        previous_to_remove_node,
                        {
                            model_name: [
                                {
                                    "filter_on": {
                                        **current.to_remove.filter_on,
                                        reverse_side: OuterRef(unique_alias),
                                    },
                                    "remove": (
                                        None
                                        if len(internal_exclude) == 0
                                        else Func.make_ors(
                                            [
                                                Q(id=OuterRef(prev))
                                                for prev in internal_exclude
                                            ]
                                        )
                                    ),
                                }
                ...
                new_query, node_to_remove, alias_of_child = current._smart_apply(
                    previous_query,
                    get_unique_alias,
                    new_model,
                    new_path,
                ...
                record_aligned: QuerySet = reduce(
                    lambda _query, _to_remove: _query.exclude(
                        **{alias_of_child: F(_to_remove)}
                    ),
                    internal_exclude,
                    new_query,
                )
                return (
                    record_aligned,
                    node_to_remove,
                    [*internal_exclude, alias_of_child],
                )
        ...
        def _reduce_by_rel(
            ...
            modified_query, modified_to_remove, _ = reduce(
                _reduce_by_key(rel_name),
                sorted(by_rels, key=lambda pred: int(pred.to_remove is not None)),
                (accum[0], accum[1], []),
            )
    ...
    def apply_to_query(self, base_name: str) -> QuerySet:
        ...
        if not self.filters:
            return query
        query, to_remove, alias = self._smart_apply(
            query, getter, base_model, None, aliases=[]
        )
        if to_remove:
...
# These fields are ignored during cloning, and during matching.
SPECIAL_TREE_FIELDS_TO_SKIP = [
    "nodenumber",
    "highestchildnodenumber",
    # Otherwise, we'd always be restricted to a subtree...
    "parent_id",
...
def safe_fetch(model: Model, filters, version):
    ...
    try:
        ...
        raise ContetRef(
            f"Object {filters} at {model._meta.model_name} is no longer present in the database"
        )
    ...
    if (
        ...
        raise ContetRef(
            f"Object {filters} of {model._meta.model_name} is out of date. Please re-run the query"
        )
...
def resolve_reference_attributes(
    ...
    all_fields = [
        ...
        if field.concrete
        and (field.attname not in fields_to_skip and field.name not in fields_to_skip)
    ]
