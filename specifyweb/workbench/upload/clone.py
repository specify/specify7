...
@transaction.atomic()
def clone_record(reference_record, inserter: Callable[[Model, Dict[str, Any]], Model],  one_to_ones={}, to_ignore: List[str] = [], override_attrs={}) -> Model:
    model: Model = type(reference_record) # type: ignore
    model_name = model._meta.model_name
    assert model_name is not None
    specify_model: Table = model.specify_model # type: ignore
    ...
    assert model_name.lower() in FIELDS_TO_NOT_CLONE, f"Schema mismatch detected at {model_name}"

    fields_to_ignore = [*[field.lower() for field in FIELDS_TO_NOT_CLONE[model_name.lower()]], *to_ignore, *GENERIC_FIELDS_TO_SKIP]

    all_fields = [field for field in model._meta.get_fields() if field.name not in fields_to_ignore]

    marked = [(field, (field.is_relation and specify_model.get_relationship(field.name).dependent or field.name.lower() in one_to_ones.get(model_name.lower(), []) and field.name is not None)) for field in all_fields]
    ...
    to_many_cloned = [[clone_record(to_many_record, inserter, one_to_ones, override_attrs = {field.remote_field.attname: inserted.pk}) # type: ignore
         for to_many_record in getattr(reference_record, field.name).all() # Clone all records separatetly
        ] for (field, is_dependent) in marked if is_dependent and not field.concrete] # Should be a relationship, but not on our side
