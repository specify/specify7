...
def upload(self, collection_id: int, uploading_agent_id: int, ds_id: int, no_commit: bool, allow_partial: bool) -> None:
    ...
    with transaction.atomic():
        ...
        if not (lock.info['operation'] == ("validating" if no_commit else "uploading")): raise AssertionError(
            f"Invalid status '{lock.info['operation']}' for upload. Expected 'validating' or 'uploading'",
            {"uploadStatus" : lock.info['operation'],
             "operation" : "upload",
...
def unupload(self, ds_id: int, agent_id: int) -> None:
    ...
    with transaction.atomic():
        ...
        if not (lock.info['operation'] == "unuploading"): raise AssertionError(
            f"Invalid status '{lock.info['operation']}' for unupload. Expected 'unuploading'",
            {"uploadStatus" : lock.info['operation'],
             "operation" : "unupload",
