...
def remove_from_recordsets(sender, obj):
    if not hasattr(sender, 'specify_model'):
        return
    if sender in (models.Workbenchtemplate, models.Workbenchrow, models.Workbenchdataitem,
                  models.Workbenchtemplatemappingitem, models.Workbenchrowimage):
        return
...
def recordset_pre_delete(recordset):
    ...
    cursor.execute(
        "delete from recordsetitem where recordsetid = %s", [recordset.id])
