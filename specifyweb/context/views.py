...
def merge_endpoint_schemas(autogenerated, hardcoded, is_strict:bool):
    ...
    params:
        ...
        is_strict:
            Whether an endpoint has strict mode one
            Strict mode runs validation against the OpenAPI schema on each
            request and responds with 400 on requests that don't correspond to
            schema
    ...
    merged_endpoints = { name: {
        ...
        **data,
        "responses": {
            **autogenerated['get']['responses'],
            **({
                   "400": {
                       "description":
                           "Returned on malformed requests whose parameters "
                           "do not correspond to expected data types."
                   }
               } if is_strict else { }),
            **(hardcoded['responses'] if 'responses' in hardcoded else { })
        }
...
def merge_components(components, endpoint_components):
    ...
    return {
        subspace_name: {
            **(
                  components[subspace_name]
                  if subspace_name in components
                  else {}
            ),
            **(
                endpoint_components[subspace_name]
                if subspace_name in endpoint_components
                else {}
            ),
        } for subspace_name in
        { *components.keys(), *endpoint_components.keys() }
    }
