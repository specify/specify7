        ...
        if isinstance(p, URLPattern):
            endpoints_schema = {
                ...
                'get': {
                    **({
                            'summary': 'TBD',
                            'description': getattr(p.callback, '__doc__')
                       } if hasattr(p.callback,'__doc__') else { }),
                    'tags': [tag],
                    'responses': {
                        '200': {
                            'description': 'TBD'
            ...
            yield (
                complete_path,
                merge_endpoint_schemas(
                    endpoints_schema,
                    getattr(p.callback,'__schema__')
                ) if hasattr(p.callback, '__schema__') else endpoints_schema
            )
        else:
