...
def return_loan_preps(collection, user, agent, data):
    ...
    tableid = spquery['contexttableid']
    assert tableid == Loanpreparation.specify_model.tableId, f"Unexpected tableId '{tableid}' in request. Expected '{Loanpreparation.specify_model.tableId}'"
...
def build_query(session, collection, user, tableid, field_specs,
    ...
    if recordsetid is not None:
        ...
        assert recordset.dbTableId == tableid, f"Unexpected tableId '{tableid}' in request. Expected '{recordset.dbTableId}'"
