    ...
    for user in Specifyuser.objects.all():
        ...
        for collection in Collection.objects.all():
            if user.usertype == 'Manager':
                user.roles.create(role=Role.objects.filter(collection=collection, name="Collection Admin").first())
            if user.usertype == 'FullAccess':
                user.roles.create(role=Role.objects.filter(collection=collection, name="Full Access - Legacy").first())
            if user.usertype in ('LimitedAccess', 'Guest'):
                user.roles.create(role=Role.objects.filter(collection=collection, name="Read Only - Legacy").first())
