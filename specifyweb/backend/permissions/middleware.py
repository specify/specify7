...
from .permissions import PermissionsException, CollectionAccessPT, \
...
class PermissionsMiddleware:
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        skip_collection_access_check = getattr(
            view_func, '__skip_sp_collection_access_check', False)
        user = request.specify_user
        col = request.specify_collection
        if not skip_collection_access_check and user != None and col != None:
            try:
                check_permission_targets(
                    col.id, user.id, [CollectionAccessPT.access])
