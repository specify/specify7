...
from specifyweb.backend.redis_cache.datatypes import RedisList
...
class RedisQueue[T]:
    def __init__(self, connection: RedisConnection, key: str,
        ...
        self.deserializer = deserializer or cast(
            Deserializer[T], default_deserializer)

    def key_name(self, *name_parts: str | None):
        key_name = "_".join(
            [self.key, *(part for part in name_parts if part is not None)])
        return key_name

    def push(self, *objs: T, sub_key: str | None = None) -> int:
        key_name = self.key_name(sub_key)
        return self.connection.right_push(*self._serialize_objs(*objs), key=key_name)
    ...
    def wait_and_pop(self, timeout: int = 0, sub_key: str | None = None) -> T:
        key_name = self.key_name(sub_key)
        popped = self.connection.blocking_left_pop(timeout, key=key_name)
        if popped is None:
    ...
    def peek(self, sub_key: str | None = None) -> T | None:
        key_name = self.key_name(sub_key)
        top_value = self._deserialize_objs(
            *self.connection.range(0, 0, key=key_name))
        if len(top_value) == 0:
            return None
        return top_value[0]
