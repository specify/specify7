...
from specifyweb.backend.workbench.upload.upload_table import (
    BulkBatchEditContext,
    BulkBatchEditFallback,
    UploadTable,
)
...
class UpdateTests(UploadTestsBase):
    def make_basic_update_plan(self):
        plan_json = {
    ...
    def test_bulk_batch_edit_preserves_update_metadata_and_audit(self):
        ...
        modified_agent_ids = [record.modifiedbyagent_id for record in records]
        modified_timestamps = [record.timestampmodified for record in records]
        rows = [
            {"Catno": "1", "Remarks field": "Changed one"},
            {"Catno": "2", "Remarks field": "Already current"},
            {"Catno": "3", "Remarks field": "Changed three"},
        ]
        batch_edit_packs = [
            {"self": {"id": record.id, "version": record.version, "ordernumber": None}}
            for record in records
        ]

        original_flush = BulkBatchEditContext.flush
        flushed_intent_counts: list[int] = []

        def tracking_flush(context):
            flushed_intent_counts.append(len(context.intents))
            return original_flush(context)

        with patch.object(BulkBatchEditContext, "flush", tracking_flush):
            results = do_upload(
                self.collection,
                rows,
                plan,
        ...
        self.assertEqual([2], flushed_intent_counts)
        ...
        self.assertEqual(records[0].remarks, "Changed one")
        self.assertEqual(records[1].remarks, "Already current")
        self.assertEqual(records[2].remarks, "Changed three")
        ...
        for index in (0, 2):
            self.assertEqual(records[index].version, versions[index] + 1)
            self.assertEqual(records[index].modifiedbyagent_id, self.agent.id)
            self.assertGreater(
                records[index].timestampmodified,
                modified_timestamps[index],
            )

            entry = lookup_in_auditlog("Collectionobject", records[index].id).get()
            self.assertEqual(entry.action, auditcodes.UPDATE)
            self.assertEqual(entry.recordversion, versions[index])
            self.assertEqual(entry.createdbyagent_id, self.agent.id)
            audit_field = Spauditlogfield.objects.get(spauditlog=entry)
            self.assertEqual(audit_field.fieldname, "remarks")
            self.assertEqual(audit_field.oldvalue, expected_old_values[index])
            self.assertEqual(audit_field.newvalue, rows[index]["Remarks field"])

        self.assertEqual(records[1].version, versions[1])
        self.assertEqual(records[1].modifiedbyagent_id, modified_agent_ids[1])
        self.assertEqual(records[1].timestampmodified, modified_timestamps[1])
    ...
    def test_bulk_batch_edit_picklist_addition_falls_back_before_queueing(self):
        ...
        record = get_table("Collectionobject").objects.create(
            catalognumber="1".rjust(9, "0"),
            text1="marsh",
            collection=self.collection,
        )

        def fail_if_bulk_queue_is_reached(*args, **kwargs):
            self.fail("Picklist fallback should run before bulk update queueing.")

        with patch.object(
            BulkBatchEditContext,
            "queue_update",
            fail_if_bulk_queue_is_reached,
        ):
            results = do_upload(
                self.collection,
        ...
        self.assertEqual(
            ["River"],
            [addition.value for addition in results[0].record_result.picklistAdditions],
        )
        record.refresh_from_db()
        self.assertEqual(record.text1, "River")
        self.assertTrue(
            get_table("Picklistitem").objects.filter(
                picklist=habitat,
                title="River",
                value="River",
            ).exists()
    ...
    def test_bulk_batch_edit_tree_models_fallback_before_mutation(self):
        ...
        context = BulkBatchEditContext(None, self.agent)

        class AuditorShouldNotBeReached:
            def pre_log(inner_self, obj, action_name):
                self.fail("Tree models should fallback before audit pre_log.")

        with self.assertRaises(BulkBatchEditFallback):
            context.queue_update(
                AuditorShouldNotBeReached(),
                taxon,
                [
                    {
                        "field_name": "name",
                        "old_value": "Original taxon",
                        "new_value": "Changed taxon",
                    }
                ],
                {"name": "Changed taxon"},
            )

        self.assertEqual(taxon.name, "Original taxon")
