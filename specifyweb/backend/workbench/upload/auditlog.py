...
class AuditLog:
    ...
    def isAuditing(self):
        ...
        if self._auditing is None or self._lastCheck is None or time() - self._lastCheck > self._checkInterval:
            ...
            do_field_updates = get_remote_pref('auditing.audit_field_updates')
            if do_field_updates is None:
                self._auditingFlds = True
            else:
                self._auditingFlds = False if do_field_updates.lower() == 'false' else True
            self.purge()
    ...
    def purge(self):
        audit_lifespan_pref = get_global_pref('AUDIT_LIFESPAN_MONTHS')

        logger.info("checking to see if purge is required")
        if audit_lifespan_pref is not None:
            cursor = connection.cursor()
            sql = "delete from spauditlogfield where date_sub(curdate(), Interval " +  audit_lifespan_pref.lower()+ " month) > timestampcreated"
            logger.info("purging audit log: %s", [sql]);
            cursor.execute(sql)
            sql = "delete from spauditlog where date_sub(curdate(), Interval " +  audit_lifespan_pref.lower()+ " month) > timestampcreated"
            logger.info("purging audit log: %s", [sql]);
            cursor.execute(sql)
        return True
