...
describe("getScopedAppResources", ()=>{
    ...
    test("no directory case", ()=>{
        ...
        const scoped = getScopedAppResources(({...staticAppResources, directories: [], disciplines: testDisciplines}))
        ...
        expect(scoped[0].directory?.id).toBe(undefined);
        expect(scoped[0].directory?.discipline).toBe(testDisciplines[0].resource_uri);
    ...
    test("directory case", ()=>{
        const newDirectory = [staticAppResources.directories[0], {...staticAppResources.directories[1], scope: 'discipline', collection: null}] as const;
        ...
        const scoped = getScopedAppResources(({...viewSetAdjusted, directories: newDirectory, disciplines: testDisciplines}))
