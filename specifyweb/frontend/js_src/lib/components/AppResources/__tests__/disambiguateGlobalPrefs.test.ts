...
describe('disambiguateGlobalPrefs', () => {
  test('no preference app resources', () => {
    ...
    expect(
      disambiguateGlobalPrefs(appResources, staticAppResources.directories)
    ).toEqual(appResources);
  });

  test('preference app resources (global prefs)', () => {
    expect(
      disambiguateGlobalPrefs(
        staticAppResources.appResources,
        staticAppResources.directories
      )
    ).toEqual(
      replaceItem(
  ...
  test('preference app resources (remote prefs)', () => {
    ...
    expect(
      disambiguateGlobalPrefs(staticAppResources.appResources, directories)
    ).toEqual(
      replaceItem(
        staticAppResources.appResources as AppResourcesTree[number]['appResources'],
        1,
  ...
  test('preference app resources (unmatched spappresourcedir)', () => {
    const appResources = staticAppResources.appResources.map((resource) => ({
      ...resource,
      spAppResourceDir: getResourceApiUrl('SpAppResourceDir', 4),
    }));
