...
describe("getUserResources", ()=>{
    ...
    test("no directory case", ()=>{
        ...
        expect(scopedResources.length).toBe(
            staticAppResources.users.length
        );
        ...
        scopedResources.forEach((resource, index)=>{
            ...
            expect(resource.directory?.discipline).toBe(testDisciplines[0].resource_uri);
            ...
            expect(resource.key).toBe(`collection_${staticAppResources.collections[0].id}_user_${user.id}`);
