...
describe('useAppResourceData', () => {
  ...
  test('data for existing resource', async () => {
    renderHook(() =>
      useAppResourceData(staticAppResources.appResources[0], undefined)
    );

    await waitFor(() => {
      expect(mockFunction).toHaveBeenCalledTimes(1);
      expect(mockFunction.mock.lastCall[0]).toEqual(spAppResourceData);
    });
  });
  ...
  test('data for new resource', async () => {
    ...
    renderHook(() =>
      useAppResourceData(serializeResource(appResource), 'this is new value')
    );

    await waitFor(() => {
      expect(mockFunction).toHaveBeenCalledTimes(1);
      expect(mockFunction.mock.lastCall[0].data).toBe('this is new value');
    });
  });
});
