...
const makeTree = (
    ...
    nodes.map((node)=>makeAppResourceNode(
        ...
        makeTree(node.children, labelIncrementor, keyIncrementor, idIncrementor, addResources, forceGenerator),
        addResources ? 
            Array.from(({length: node.appResources ?? 0})).map(
            ()=>addMissingFields("SpAppResource", {id: (idIncrementor.next().value as number)})
            ) 
            : [],
        addResources ? 
            Array.from(({length: node.viewSets ?? 0})).map(
            ()=>addMissingFields("SpViewSetObj", {id: (idIncrementor.next().value as number)})
            ) 
            : [],
    ))
);
