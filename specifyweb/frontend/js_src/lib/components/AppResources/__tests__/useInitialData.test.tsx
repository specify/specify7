...
describe('useInitialData', () => {
  ...
  overrideAjax(
    getResourceApiUrl('SpAppResourceData', spAppResourceDataId),
    spAppResourceData
  );

  test('case: spAppResourceData', async () => {
    const { result } = renderHook(() =>
      useInitialData(resource, spAppResourceDataId, undefined)
    );
  ...
  test('case: using static template file', async () => {
    ...
    const { result } = renderHook(() =>
      useInitialData(resource, undefined, templateFile)
    );
  ...
  test('case: using template', async () => {
    const { result } = renderHook(() =>
      useInitialData(staticAppResources.appResources[2], undefined, undefined)
    );
