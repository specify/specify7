...
describe("getUserTypeResources", ()=>{
    ...
    test("no directory case", ()=>{
        ...
        expect(scopedResources.length).toBe(4);
        scopedResources.forEach((resource, index)=>{
            const userType = userTypes[index];
            expect(resource.label).toBe(userType);
            expect(resource.key).toBe(`collection_${staticAppResources.collections[0].id}_userType_${userType}`);
            expect(resource.directory?.id).toBe(undefined);
            expect(resource.directory?.discipline).toBe(
            testDisciplines[0].resource_uri
            );
            expect(resource.directory?.collection).toBe(staticAppResources.collections[0].resource_uri);
            expect(resource.directory?.userType?.toLowerCase()).toBe(userType.toLowerCase());
    ...
    test("directory case", ()=>{
        ...
        scopedResources.forEach((resource, index)=>{
            ...
            if (index == 0){
                ...
                expect(resource.appResources.length).toBe(1);
            }else{
                expect(resource.directory?.id).toBe(undefined);
                expect(resource.appResources.length).toBe(0);   
            }

            expect(resource.viewSets.length).toBe(0);
        });
