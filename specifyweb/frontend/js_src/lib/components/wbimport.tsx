...
function ChooseFile(props: {fileType: FileType, update: HandleAction}) {
	...
	return (
		<p>Choose {props.fileType} file: <input type="file" accept={exts} onChange={event => changed(event.target)} />
		</p>
	);
}
	...
	return (
		<p>
			...
			<input type="text" value={props.name}
				   onChange={event => props.update({type: 'SetDataSetNameAction', value: event.target.value})} />
		</p>
	);
}
...
function ToggleHeader(props: {hasHeader: boolean, update: HandleAction}) {
	return (
		<label>First row is header:{' '}
			<input type="checkbox" onChange={() => props.update({type: 'ToggleHeaderAction'})}
