...
// import { getSelectedLocalities, WbGeoLocate } from './WbToolkit/GeoLocate';
...
function Navigation({
  ...
  const handlePrevious = () => {
    ...
    utils.navigateCells(name, 'previous');
  }
  const handleNext = () => {
    setCurrentPosition((prev) => Math.min(totalCount, prev + 1));
    utils.navigateCells(name, 'next');
  }
  return (
    <span
      aria-atomic
...
export class WbUtilsReact {
  ...
  constructor(private readonly workbench: Workbench) {
    const debounced = _.debounce(
      this.searchCells,
      Math.ceil(clamp(10, this.workbench.data.length / 20, 200)),
      false
    ).bind(this);
    // Workaround for _.debounce not working with async functions
    // this.searchCells = async (event): Promise<void> => debounced(event, null);
  }

  navigateCells(
    type: keyof WbCellCounts | null,
    ...
    // );
    const initialCellRelativePosition = 0;
    // if (
  ...
  searchCells(
    ...
    const firstVisibleColumn =
      getHotPlugin(this.workbench.hot, 'autoColumnSize').getFirstVisibleColumn() -
      3;
  ...
  toggleCellTypes(
    ...
    if (spreadsheetContainer) spreadsheetContainer.classList[action](cssClassName);
  }  
  ...
  replaceCells(event: KeyboardEvent, replacementValueElement: HTMLInputElement) {
    if (
      event.key !== 'Enter' ||
