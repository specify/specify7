...
export class WbCellMetaReact {
  ...
  updateCellMeta<KEY extends keyof WbMeta>(
    ...
    const cell = initialCell ?? this.workbench.hot.getCell(visualRow, visualCol);
  ...
  public getCellMetaObject(): RA<RA<WbMetaArray>> {
    if (this.flushIndexedCellData || this.indexedCellMeta === undefined) {
      ...
      const resolveIndex = (
        ...
        (this.workbench.searchPreferences.navigation.direction ===
          'rowFirst') ===
        first
          ? visualRow
          : visualCol;
