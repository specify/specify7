...
export class DisambiguationReact {
  public constructor(
  ...
  public isAmbiguousCell(): boolean {
    if (this.workbench.mappings === undefined || this.workbench.hot === undefined)
      return false;
  ...
  public cellWasDisambiguated(
    ...
    return Boolean(
      this.workbench.validation?.uploadResults.ambiguousMatches[physicalRow]?.find(
        ({ physicalCols, mappingPath }) =>
          physicalCols.includes(physicalCol) &&
  ...
  public afterChangeDisambiguation(physicalRow: number): void {
    (this.workbench.validation?.uploadResults.ambiguousMatches[physicalRow] ?? [])
      ...
      .forEach((physicalCol) =>
        this.workbench.cells?.recalculateIsModifiedState(physicalRow, physicalCol)
      );
