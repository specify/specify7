...
describe("fetchAttachmentParent", ()=>{
    ...
    test("fetches related parent correctly", async ()=>{
        ...
        const collectionObjectAttachment = new tables.CollectionObjectAttachment.Resource({
            id: collectionObjectAttachmentId,
            collectionObject: getResourceApiUrl("CollectionObject", collectionObject.id),
        });

        collectionObjectAttachment.set("attachment", attachment);

        const parent = await fetchAttachmentParent(tables.CollectionObject, serializeResource(attachment));
