...
describe('fetchOriginalUrl', () => {
  ...
  test('url when token not required for get', async () => {
    ...
    expect(url).toBe(
      'http://host.docker.internal/fileget?coll=KU+Fish+Voucher+Collection&type=O&filename=testLocation&downloadname=testSlash.jpg'
    );
  });
  ...
  test('url when token is required for get', async () => {
    ...
    expect(url).toBe(
      'http://host.docker.internal/fileget?coll=KU+Fish+Voucher+Collection&type=O&filename=testLocation&downloadname=testSlash.jpg&token=testToken'
    );
  });
});
