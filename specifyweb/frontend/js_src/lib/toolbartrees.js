...
var TreeListDialog = Backbone.View.extend({
    ...
    _render: function(trees) {
        const tasks = userInfo.available_tasks.map(task => task.toLowerCase());
        var entries = trees.filter(t => tasks.includes(t + 'tree')).map(t => this.dialogEntry(t));
...
module.exports =  {
    ...
    disabled(userInfo) {
        const tasks = userInfo.available_tasks.map(task => task.toLowerCase());
        return treesForAll.concat(treesForPaleo).every(name => !tasks.includes(name + 'tree'));
    },
    execute: function() {
