...
define([
  ...
  return function () {
    ...
    _(uitypes).each(function (uitype) {
      _([true, false]).each(function (doingFormTable) {
        var node =
          '<cell type="field" id="1" name="aField" uitype="' + uitype + '"/>';
        test(node + ' doingFormTable: ' + doingFormTable, function () {
          var result = instProcessCell(doingFormTable, node);
    ...
    _(uitypes).each(function (uitype) {
      _([true, false]).each(function (doingFormTable) {
        var node =
          '<cell type="field" id="1" name="aField" uitype="' +
          uitype +
          '" isrequired="true" initialize="somedata"/>';
        test(node + ' doingFormTable: ' + doingFormTable, function () {
          var result = instProcessCell(doingFormTable, node);
          var control = result.find('.specify-field');
          equal(control.length, 1, 'exactly one specify-field element');
      ...
      _(['', 'someLabel', null]).each(function (labelStr) {
        ...
        test(node + ' doingFormTable: ' + doingFormTable, function () {
          ...
          if (_.isNull(labelStr)) {
            ...
            else equal(label.text(), '', 'empty label text');
          } else {
            if (doingFormTable)
              equal(labelOR, labelStr, 'label override is correct');
            else equal(label.text(), labelStr, 'label text is correct');
          }
        });
      });
    });
    ...
    _([true, false]).each(function (doingFormTable) {
      _(['rows="10"', 'rows="3"', '']).each(function (rowsAttr) {
        var node =
          '<cell type="field" id="4" name="remarks" uitype="textarea" ' +
          rowsAttr +
          ' colspan="3"/>';
        test(node + ' doingFormTable: ' + doingFormTable, function () {
          var result = instProcessCell(doingFormTable, node);
    ...
    _([true, false]).each(function (doingFormTable) {
      _(['rows="10"', 'rows="3"', '']).each(function (rowsAttr) {
        var node =
          '<cell type="field" id="2" name="brieftext" uitype="textareabrief" ' +
          rowsAttr +
          ' colspan="3"/>';
        test(node + ' doingFormTable: ' + doingFormTable, function () {
          var result = instProcessCell(doingFormTable, node);
    ...
    _([true, false]).each(function (doingFormTable) {
      _([true, false]).each(function (givePickList) {
        var node =
          '<cell type="field" id="7" name="prepType" uitype="combobox" ' +
    ...
    _([true, false]).each(function (doingButton) {
      _([true, false]).each(function (doingAlign) {
        ...
        var node =
          '<cell type="subview" viewname="Authors" id="10" name="authors" ' +
          init +
          ' defaulttype="table" colspan="12" rows="3"/>';
        test(node, function () {
          var result = instProcessCell(false, node);
