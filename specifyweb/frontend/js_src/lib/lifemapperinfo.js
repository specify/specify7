...
module.exports = Backbone.View.extend({
    ...
    showCOMap(dialog, occurrence_name){
                        ...
                        new Promise(resolve =>
                            collection_object.rget('collectingevent.locality').done(locality =>
                                Leaflet.getMarkersFromLocalityResource(
                                    locality,
                                    this.model.get('id') === collection_object.get('id') ?
        ...
        $.get(format_occurrence_map_request(occurrence_name)).done(response => {
            ...
            const [map, layer_group] = Leaflet.showCOMap(
    ...
    showCOCount: (dialog, source_name, occurrence_name) =>
        $.get(format_occurrence_count_request(source_name, occurrence_name))
            .done(response => (
                response.count === 0 ?
                    '' :
