...
export const statsText = createDictionary({
  ...
  showPreparationsTotal: {
    ...
    'de-ch': 'Gesamtanzahl der Vorbereitungen anzeigen',
    'es-es': 'Totales de preparativos del espectáculo',
    'fr-fr': 'Afficher les totaux des préparatifs',
    'pt-br': 'Totais dos preparativos para o show',
    'ru-ru': 'Итоги подготовки к шоу',
    'uk-ua': 'Підсумки підготовки до шоу',
  },
  showPreparationsTotalDescription: {
    ...
    'de-ch':
      'Ist diese Option aktiviert, zeigt das Standard-Statistikfeld für Zubereitungen neben der Gesamtzahl auch die Anzahl der einzelnen Zubereitungen eines bestimmten Zubereitungstyps an. Dies ist insbesondere für chargenbasierte Sammlungen hilfreich.',
    'es-es':
      'Si está habilitado, el panel de estadísticas de preparaciones predeterminado incluirá un recuento total de cada preparación de un tipo específico, junto con el total general. Esto resulta útil para recolecciones basadas en lotes.',
    'fr-fr':
      "Si cette option est activée, le panneau de statistiques par défaut des préparations affichera le nombre total pour chaque préparation d'un type donné, en plus du total général. Ceci est utile pour les collections par lots.",
    'pt-br':
      'Se ativado, o painel de estatísticas de preparações padrão incluirá a contagem total de cada preparação de um determinado tipo, além do total geral. Isso é útil para coleções baseadas em lotes.',
    'ru-ru':
      'Если эта функция включена, на панели статистики по препаратам по умолчанию будет отображаться общее количество каждого препарата определенного типа наряду с общим итогом. Это полезно для коллекций, основанных на партиях.',
    'uk-ua':
      'Якщо ввімкнено, панель статистики препаратів за замовчуванням включатиме загальну кількість для кожного препарату певного типу препарату поряд із загальною кількістю. Це корисно для колекцій на основі партій.',
  },
  autoRefreshRate: {
