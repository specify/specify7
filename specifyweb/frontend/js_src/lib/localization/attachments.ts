...
export const attachmentsText = createDictionary({
  ...
  matchByFilenameDescription: {
    'en-us': 'Parse the record identifier from each filename using a field formatter. Works best with structured, numeric fields like Catalog Number.',
  ...
  matchByMappingFileDescription: {
    'en-us': 'Use an explicit CSV mapping file that lists which record each file belongs to. No filename parsing required — works with any field format, including free text.',
    'de-ch': 'Verwenden Sie eine explizite CSV-Zuordnungsdatei, die auflistet, zu welchem Datensatz jede Datei gehört. Keine Dateinamenanalyse erforderlich.',
    'es-es': 'Use un archivo de mapeo CSV explícito que indique a qué registro pertenece cada archivo. No se requiere análisis de nombres de archivo.',
    'fr-fr': "Utilisez un fichier CSV de mappage explicite qui répertorie à quel enregistrement appartient chaque fichier. Aucune analyse du nom de fichier requise.",
    'ru-ru': 'Используйте явный CSV-файл сопоставления, в котором указано, к какой записи относится каждый файл. Анализ имени файла не требуется.',
    'uk-ua': 'Використовуйте явний CSV-файл зіставлення, у якому вказано, до якого запису належить кожен файл. Аналіз назви файлу не потрібен.',
    'pt-br': 'Use um arquivo CSV de mapeamento explícito que lista a qual registro cada arquivo pertence. Nenhuma análise de nome de arquivo é necessária.',
    'hr-hr': 'Koristite eksplicitnu CSV datoteku mapiranja koja navodi kojem zapisu pripada svaka datoteka. Nije potrebna analiza naziva datoteke.',
    nb: 'Bruk en eksplisitt CSV-tilordningsfil som viser hvilken post hver fil tilhører. Ingen filnavnanalyse kreves.',
  },
  matchValue: {
    'en-us': 'Match Value',
    'de-ch': 'Abgleichswert',
