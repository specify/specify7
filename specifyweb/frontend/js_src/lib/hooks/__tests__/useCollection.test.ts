...
describe('useCollection', () => {
  ...
  const collectingEventAttributeUrl = getResourceApiUrl(
  ...
  const castAsCollection = (data: ReturnType<typeof useCollection>[0]) =>
    data as Collection<AnySchema>;
  ...
  test('to-many dependent collection gets fetched and set correctly', async () => {
    ...
    const collector =
      tables.CollectingEvent.strictGetRelationship('collectors');
  ...
  test('to-many independent collection gets fetched and set correctly', async () => {
    ...
    const collectionObject =
      tables.CollectingEvent.strictGetRelationship('collectionobjects');
  ...
  test('to-one independent (exists) and filter ', async () => {
    ...
    const collectingTrip =
      tables.CollectingEvent.strictGetRelationship('collectingtrip');
  ...
  test('to-one independent (does not exist, and no backref) and filter ', async () => {
    const collectingEvent = new tables.CollectingEvent.Resource({ id: ceID });
    const discipline =
      tables.CollectingEvent.strictGetRelationship('discipline');
  ...
  test('to-one dependent (parent exists)', async () => {
    const collectingEvent = new tables.CollectingEvent.Resource({ id: ceID });
    const collectingEventAttribute =
      tables.CollectingEvent.strictGetRelationship('collectingeventattribute');
