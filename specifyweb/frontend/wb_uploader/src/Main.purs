...
addToOneWidget tables spTable (UploadTable ut) = do
  let
    undefinedRels = filter (\r -> r.type == SpManyToOne && (isNothing $ M.lookup r.name ut.toOneDefs)) spTable.relationships
  selected <- selectRelWidget undefinedRels
  case getRelatedTable tables selected of
    Nothing -> D.text $ "bad relationship: " <> selected.name
    Just relatedTable -> do
...
fieldsWidget spTable ut@(UploadTable { fieldDefs }) = D.ul' $ (showField <$> fieldDefs) <> [ D.li' [ addFieldWidget spTable ut ] ]
  where
  showField (Tuple name source) = case source of
...
addFieldWidget spTable (UploadTable ut) = do
  let
    unassignedFields = filter (\f -> isNothing $ lookup f.name ut.fieldDefs) spTable.fields
