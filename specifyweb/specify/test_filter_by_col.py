...
class TestFilterByCol(TestCase):
    ...
    def test_table_field_paths(self) -> None:
        for table, target_paths in TABLE_TO_COLLECTION_FIELD_PATHS.items():
            sp_table = spmodels.datamodel.get_table(table)
            paths = build_query_paths(sp_table)
            for i in range(len(paths)):
                self.assertEqual(paths[i], target_paths[i])

    def test_table_field_paths_2(self) -> None:
        for table, target_paths in TABLE_TO_COLLECTION_FIELD_PATHS.items():
            sp_table = spmodels.datamodel.get_table(table)
            print('\n' + table)
            paths = build_query_paths(sp_table)
    ...
    def test_table_field_paths_3(self) -> None:
        table = 'SpPermission'
        sp_table = spmodels.datamodel.get_table(table)
        print('\n' + table)
        paths = build_query_paths(sp_table)
    ...
    def test_table_field_paths_all_tables(self) -> None:
        for sp_table in spmodels.datamodel.tables:
            print('\n' + sp_table.name)
            paths = build_query_paths(sp_table)
            if paths is not None:
    ...
    def test_table_field_paths_all_tables_2(self) -> None:
        x = 'TABLE_TO_COLLECTION_FIELD_PATHS = {\n'
        for sp_table in spmodels.datamodel.tables:
            x += f"    '{sp_table.name}': {build_query_paths(sp_table)},\n"
            # paths = build_query_paths(sp_table)
            # if paths is not None:
            #     for path in paths:
            #         print(path)
    ...
    def test_build_query_exprs_1(self) -> None:
        table = 'SpPermission'
        sp_table = spmodels.datamodel.get_table(table)
        queryset = None
        print(sp_table.name)
        ...
        if paths is not None:
            for path in paths:
                print(path)
        print('->')
        exprs = build_query_exprs(sp_table)
        for expr in exprs:
            print(expr)
        print()

    def test_build_query_exprs_all_tables(self) -> None:
        for sp_table in spmodels.datamodel.tables:
            # if sp_table.name in {'DisposalAgent', 'SpPermission'}:
            if sp_table.name in {}:
                continue
            queryset = None
            print(sp_table.name)
    ...
    def test_create_test_data(self) -> None:
        print("TABLE_TO_COLLECTION_FIELD_PATHS = {")
        for sp_table in spmodels.datamodel.tables:
            paths = build_query_paths(sp_table)
            if paths is None:
                print(f"    '{sp_table.name}': None,")
                continue
            print(f"    '{sp_table.name}': {{")
            print(f"        'field_paths': [")
            for path in paths:
                print(f"            {path},")
            print("        ],")
