...
@openapi(schema={
    'post': {
        ...
        "responses": {
            "200": {
                "description": "Recordset merge successful",
                "content": {
                    "application/json": {
                        "schema": {
                            "type": "object",
                    ...
                    "application/json": {
                        "schema": {
                            ...
                            "properties": {
                                "error": {
                                    "type": "string",
                                    "description": "All recordsets must have the same tableid"
                                }
                            }
                        }
                    }
                    ...
                    "application/json": {
                        "schema": {
                            ...
                            "properties": {
                                "error": {
                                    "type": "string",
                                    "description": "No valid recordsets found"
                                }
                            }
                        }
                    }
...
@login_maybe_required
@require_POST
def recordset_merge(request: http.HttpRequest) -> http.JsonResponse:
    try:
        ...
        if not recordset_ids:
            return http.JsonResponse({'error': 'No recordset IDs provided'}, status=400)

        # Get the recordset records for the given IDs from the RecordSet table
        recordsets = spmodels.Recordset.objects.filter(id__in=recordset_ids)

        if not recordsets.exists():
            return http.JsonResponse({'error': 'No valid recordsets found'}, status=404)

        # Verify that the 'tableid' field in all the recordsets is the same
        tableid = recordsets.first().tableid
        if not all(recordset.tableid == tableid for recordset in recordsets):
            return http.JsonResponse({'error': 'All recordsets must have the same tableid'}, status=400)

        # Get all the recordsetitems for the given recordsets
        recordsetitems = spmodels.Recordsetitem.objects.filter(recordset__in=recordsets)
        ...
        with transaction.atomic():
            ...
            new_recordset = spmodels.Recordset.objects.create(
                tableid=tableid,
                name='Merged Recordset',
                createdby=request.specify_user_agent,
                modifiedby=request.specify_user_agent,
                created=request.specify_user_agent,
                modified=request.specify_user_agent,
            )

            # Create a new recordsetitem record for each record ID in the list
            spmodels.Recordsetitem.objects.bulk_create([
                spmodels.Recordsetitem(
        ...
        return http.JsonResponse({'message': 'Recordset merge successful'}, status=200)
