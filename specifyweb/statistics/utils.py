...
def get_tree_rank_stats_str(rankid, request):
    ...
    all_node_numbers_used = [str(x[1]) for x in list(cursor.fetchall())]

    cursor.execute("""
        select taxonid from taxon where rankid = % s
        """, [rankid])
